...
public class ReportServiceImpl implements ReportService {
     ...
     * @return
     */
    public UserReportVO getUserStatistics(LocalDate begin,
                                          LocalDate end) {
    ...
    public OrderReportVO getOrdersStatistics(LocalDate begin,
                                             LocalDate end) {
        List<LocalDate> dateList = new ArrayList<>();
        dateList.add(begin);
        while (!begin.equals(end)) {
        ...
        for (LocalDate date : dateList) {
            ...
            LocalDateTime endTime = LocalDateTime.of(date,
                    LocalTime.MAX);
            Integer orderCount = getOrderCount(beginTime, endTime,
                    null);

            //查询每天的订单完成数
            Integer validOrderCount = getOrderCount(beginTime,
                    endTime, Orders.COMPLETED);
        ...
        Integer validOrderCount =
                validOrderCountList.stream().reduce(Integer::sum).get();
     ...
     * @return
     */
    public SalesTop10ReportVO getTop10(LocalDate begin,
                                       LocalDate end) {
        LocalDateTime beginTime = LocalDateTime.of(begin,
                LocalTime.MIN);
        LocalDateTime endTime = LocalDateTime.of(end,
                LocalTime.MAX);
        List<GoodsSalesDTO> salesTop10 =
    ...
    public void exportBusinessData(HttpServletResponse response) {
        ...
        InputStream resourceAsStream =
                this.getClass().getClassLoader().getResourceAsStream("template/Operational_Data_Report_Template.xlsx");
        try {
