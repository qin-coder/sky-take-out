      ...
      <div>
        <div v-if="orderData.length > 0">
          <el-table :data="orderData" stripe class="tableBox" style="width: 100%">
            ...
            <el-table-column label="Address" :class-name="dialogOrderStatus === 2 ? 'address' : ''">
              <template #default="scope">
                <div class="ellipsisHidden">
                  <el-popover
                    placement="top-start"
            ...
            <el-table-column
              ...
              align="center"
            ></el-table-column>
            <el-table-column prop="amount" label="Amount received" align="center"></el-table-column>
        ...
        <div class="order-middle">
          <div class="user-info">
            ...
            <div class="user-remark" :class="{ orderCancel: dialogOrderStatus === 6 }">
              <div>{{ dialogOrderStatus === 6 ? 'Canceled reason' : 'Remarks' }}</div>
              <span>{{
                dialogOrderStatus === 6
          ...
          <div class="dish-info">
            ...
            <div class="dish-all-amount">
              <label>Subtotal for dishes</label>
          ...
          <div class="amount-info">
            ...
            <div class="amount-list">
              ...
              <div class="pack-amount">
                <span class="amount-name">Meal box fe:</span>
                <span class="amount-price"
                  >￥{{ ((diaForm!.packAmount * 100) / 100).toFixed(2) }}</span
                >
              </div>
              <div class="all-amount">
                <span class="amount-name">Amount received:</span>
      ...
      <el-form label-width="90px">
        ...
        <el-form-item v-if="cancelReason === 'Custom Reason'" label="reason:">
          <el-input
            v-model.trim="remark"
            type="textarea"
            :placeholder="'Please input' + cancelDialogTitle + 'reason'"
            maxlength="20"
          />
        </el-form-item>
      </el-form>
...
const rejectReasonList = reactive([
  {
    value: 1,
    label: 'We are currently unable to accept new orders due to a high volume of existing orders.'
  },
  { value: 2, label: 'The dish is sold out and orders cannot be taken at the moment.' },
  { value: 3, label: 'The restaurant is closed and unable to take orders at the moment.' },
...
const cancelrReasonList = reactive([
  ...
  { value: 2, label: 'The dish is sold out and orders cannot be taken at the moment.' },
  { value: 3, label: 'Insufficient delivery personnel, unable to deliver.' },
  { value: 4, label: 'Customer phone cancellation' },
  { value: 0, label: 'Custom Reason' }
])
...
const orderList = [
  ...
  { label: 'Waiting for orders', value: 2 },
  { label: 'To be delivered', value: 3 },
...
const tabList = computed(() => [
  { label: 'Waiting for orders', value: 2, num: orderStatics.value.toBeConfirmed },
  { label: 'To be delivered', value: 3, num: orderStatics.value.confirmed }
])
...
const orderAccept = async (row: any) => {
  ...
  if (res.data.code === 0) {
    ...
    ElMessage.success('Order received')
  } else {
...
const orderReject = (row: any) => {
  ...
  orderId.value = row.id
  dialogOrderStatus.value = row.status
  cancelDialogTitle.value = 'Rejection'
  dialogVisible.value = false
  cancelReason.value = ''
}
...
const confirmCancel = async () => {
  ...
  // 判断是取消还是拒单
  const action = cancelDialogTitle.value === 'Cancel' ? orderCancelAPI : orderRejectAPI
  const payload = {
    id: orderId.value,
...
const deliveryOrComplete = async (status1: number, id: number) => {
  ...
  if (res.code === 1) {
    orderId.value = ''
    dialogVisible.value = false
    ElMessage.success(`${status1 === 3 ? 'Delivery successful' : 'Order completed'}`)
    getOrderListData(status.value)
  } else {
    // Handle error
  }
