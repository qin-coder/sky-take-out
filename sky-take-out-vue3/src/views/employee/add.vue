...
const checkAge = (rule: any, value: string, callback: (error?: Error) => void) => {
  if (value === '' || value === undefined) {
    ...
    if (age < 3) {
      callback(new Error('Age cannot be less than 3 years'))
    } else if (age > 99) {
      callback(new Error('Age cannot be greater than 99 years'))
    } else {
      callback()
    }
  }
...
const rules = {
  name: [
    ...
    { min: 2, message: 'Name length cannot be less than 2 characters', trigger: 'blur' },
    { max: 20, message: 'Name length cannot exceed 20 characters', trigger: 'blur' }
  ],
  account: [
    { required: true, trigger: 'blur', message: 'Cannot be empty' },
    {
